...
def get_itemdata_by_id(item_id, columns):
    sql = sqlalchemy.text('SELECT * FROM items WHERE item_id = :item_id')
    return list(create_sesion_and_execute(sql, {"item_id": item_id, "columns": ",".join(columns)}))
...
def get_userdata_by_user_id(user_id, columns):
    sql = sqlalchemy.text('SELECT :columns FROM users WHERE user_id = :user_id')
    return create_sesion_and_execute(sql, {"user_id": user_id, "columns": ",".join(columns)})
...
def get_user_balance(user_id):
    sql = sqlalchemy.text('SELECT balance FROM virtualcurrency WHERE user_id = :user_id')
    return create_sesion_and_execute(sql, {"user_id": user_id})
...
def update_item_v1(title, description, image_url, item_id):
    sql = sqlalchemy.text(
        'UPDATE items SET title = :title, description = :description, image_url = :image_url WHERE item_id = :item_id')
    return create_sesion_and_execute(sql, {"title": title, "description": description, "image_url": image_url,
...
def insert_item(user_id, name, description, category, price, condition, random_filename):
    sql = sqlalchemy.text(
        'INSERT INTO items (user_id, title, description, category, starting_price, current_price, condition, image_url) VALUES (:user_id, :name, :description, :category, :price, :price, :condition, :random_filename)')
    params = {"user_id": user_id, "name": name, "description": description, "category": category, "price": price,
              "condition": condition, "random_filename": random_filename}
    return create_sesion_and_execute(sql, params, True)


def insert_message(sender_id, receiver_id, message_text):
    sql = sqlalchemy.text(
        'INSERT INTO Messages (sender_id, receiver_id, message_text) VALUES (:sender_id, :receiver_id, :message_text)')
...
def select_messages(user_id, session_user_id):
    sql = sqlalchemy.text(
        'SELECT * FROM Messages WHERE (sender_id = :session_user_id AND receiver_id = :user_id) OR (sender_id = :user_id AND receiver_id = :session_user_id)')
